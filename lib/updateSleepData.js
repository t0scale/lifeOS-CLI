...
exports.updateSleepData = async (dt) => {
  try {
    // we are receving an array with both the users requested date
    // and the oura adjusted date index 0 is oura, index 1 is users date (notion date)
    console.log('from updateSleepData', dt);
    console.log('from updateSleepData', dt[0]);

    // we want bedtime_end, bedtime_start, duration (in-bed time) total (asleep time)
    const res = await axios.get(
      `https://api.ouraring.com/v1/sleep?start=${dt[0]}&end=${dt[0]}`
    );
